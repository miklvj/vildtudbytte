...
'''
SELECTIONS ON SUNBURST STORED
'''



@app.callback(
    Output('sunburst_click_depth', 'data'),
    [Input('species_sunburst', 'clickData')],
    [State('sunburst_click_depth', 'data')]
)
def update_click_depth(clickData, current_depth):
    if clickData and 'points' in clickData and len(clickData['points']) > 0:
        clicked_point = clickData['points'][0]
        # If a group (at depth 2) is currently selected and the center (itself) is clicked:
        if current_depth.get('depth') == 2 and 'parent' in clicked_point:
            if not clicked_point['parent'] and clicked_point['label'] == current_depth.get('label'):
                # The center of the sunburst (the selected group) was clicked
                return {'depth': 1}  # Reset to top level
            elif clicked_point['parent']:
                # A species (depth 2 with a parent) was clicked, maintain depth
                return {'depth': 2, 'label': current_depth.get('label')}  # Maintain current group selection
        elif not clicked_point.get('parent'):
            # A different group was clicked, update depth and label
            return {'depth': 2, 'label': clicked_point['label']}
    # No change or invalid click data, maintain current state
...
@app.callback(
    ...
    [Input("range_slider", "value"),
     ...
     Input("sunburst_click_depth", "data")]
)
def update_sunburst(selected_years, selected_kommunes, click_depth):
    filtered_df = df[
        (df['Year'] >= selected_years[0]) &
        (df['Year'] <= selected_years[1])
    ]
    if selected_kommunes:
        filtered_df = filtered_df[filtered_df['Kommune'].isin(selected_kommunes)]

    depth = click_depth.get('depth') if click_depth else None
    label = click_depth.get('label') if click_depth else None

    if depth == 2 and label:
        filtered_df = filtered_df[filtered_df['Group'] == label]
        filtered_df['Unique_Species'] = label + '-' + filtered_df['Species']
        color_column = 'Unique_Species'
    else:
        color_column = 'Group'
...
def display_map(selected_years, sunburst_click_data, selected_kommunes):
    ...
    df_grouped = df_grouped[(df_grouped['TakenGameYearStart'] > 0) & (df_grouped['TakenGameYearEnd'] > 0)]
    ...
    for kommune in selected_kommunes:
        ...
        fig.add_trace(
            go.Choroplethmapbox(
                ...
                featureidkey="properties.navn",
                z=df_kommune['Procentvis ændring'],  
